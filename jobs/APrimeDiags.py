...
class APrimeDiags(object):
    ...
    def postvalidate(self):
        ...
        if not os.path.exists(self.output_path):
            msg = 'aprime-{}-{}: output directory missing'.format(
        ...
        try:
            ...
            msg = 'aprime-{}-{}: failed to read output directory'.format(
                self.start_year, self.end_year)
            logging.error(msg)
            return False

        if not output_contents:
            msg = 'aprime-{}-{}: output directory empty'.format(
                self.start_year, self.end_year)
            logging.error(msg)
            return False
        ...
        if not self._check_links():
            ...
            msg = 'aprime-{}-{}: some links are broken in output page'.format(
                self.start_year, self.end_year)
            logging.error(msg)
    ...
    def _check_links(self):
        ...
        if not found:
            ...
            msg = 'aprime-{}-{}: found output index.html at {}'.format(
                self.start_year, self.end_year, page_path)
            logging.info(msg)
