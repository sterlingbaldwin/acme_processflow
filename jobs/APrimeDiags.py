...
class APrimeDiags(object):
    ...
    def postvalidate(self):
        ...
        error_threashold = 600  # the expected number of output files
        ...
        if output_total < error_threashold:
            msg = 'Aprime failed post validation check, only {0} out of {1} files present'.format(
                output_total, error_threashold)
            logging.info(msg)
        if output_total < error_threashold:
            return False
    ...
    def _check_links(self):
        ...
        """
        page_path = os.path.join(self.config['target_host_path'], 'index.html')
        ...
        with open(page_path, 'r') as fp:
            ...
            for link in links:
                link_path = os.path.join(page_path, link.attrs['href'])
