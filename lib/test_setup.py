...
class TestSetup(unittest.TestCase):
    ...
    def test_expected_config(self):
        ...
        args = ['-c', os.path.join(base_path, 'tests', 'test_run_no_sta.cfg'), '-f', '-r', resource_path]
    ...
    def test_config_no_white_space(self):
        ...
        resource_path = os.path.join(base_path, 'resources')
        project_path = os.path.abspath(os.path.join('..', 'testproject'))
        args = ['-c', os.path.join(base_path, 'tests', 'test_run_no_white_space.cfg'), '-f', '-n', '-r', resource_path]
        display_event = threading.Event()
        thread_kill_event = threading.Event()
        mutex = threading.Lock()
        event_list = Event_list()
    ...
    def test_config_bad_destination_endpoint(self):
        ...
        resource_path = os.path.join(base_path, 'resources')
        project_path = os.path.abspath(os.path.join('..', 'testproject'))
        args = ['-c', os.path.join(base_path, 'tests', 'test_run_bad_destination_endpoint.cfg'), '-f', '-n', '-r', resource_path]
        display_event = threading.Event()
        thread_kill_event = threading.Event()
        mutex = threading.Lock()
        event_list = Event_list()
    ...
    def test_config_bad_source_path(self):
        ...
        resource_path = os.path.join(base_path, 'resources')
        project_path = os.path.abspath(os.path.join('..', 'testproject'))
        args = ['-c', os.path.join(base_path, 'tests', 'test_run_bad_source_path.cfg'), '-f', '-n', '-r', resource_path]
        display_event = threading.Event()
        thread_kill_event = threading.Event()
        mutex = threading.Lock()
        event_list = Event_list()
    ...
    def test_config_config_doesnt_exist(self):
        ...
        resource_path = os.path.join(base_path, 'resources')
        project_path = os.path.abspath(os.path.join('..', 'testproject'))
        args = ['-c', os.path.join(base_path, 'tests', 'DOES_NOT_EXIST.cfg'), '-f', '-n', '-r', resource_path]
        display_event = threading.Event()
        thread_kill_event = threading.Event()
        mutex = threading.Lock()
        event_list = Event_list()
    ...
    def test_config_invalid_config(self):
        ...
        resource_path = os.path.join(base_path, 'resources')
        project_path = os.path.abspath(os.path.join('..', 'testproject'))
        args = ['-c', os.path.join(base_path, 'tests', 'invalid.cfg'), '-f', '-n']
        display_event = threading.Event()
        thread_kill_event = threading.Event()
        mutex = threading.Lock()
        event_list = Event_list()
    ...
    def test_bad_config_key(self):
        ...
        resource_path = os.path.join(base_path, 'resources')
        project_path = os.path.abspath(os.path.join('..', 'testproject'))
        args = ['-c', os.path.join(base_path, 'tests', 'test_run_invalid_key.cfg'), '-f', '-n']
        display_event = threading.Event()
        thread_kill_event = threading.Event()
        mutex = threading.Lock()
        event_list = Event_list()
    ...
    def test_no_global(self):
        ...
        resource_path = os.path.join(base_path, 'resources')
        project_path = os.path.abspath(os.path.join('..', 'testproject'))
        args = ['-c', os.path.join(base_path, 'tests', 'test_run_no_global.cfg'), '-f', '-n']
        display_event = threading.Event()
        thread_kill_event = threading.Event()
        mutex = threading.Lock()
        event_list = Event_list()
